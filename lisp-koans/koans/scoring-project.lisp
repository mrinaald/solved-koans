...
(defvar score-table (make-array '(7 7)))
(dotimes (x 7)
  (dotimes (y 7)
    (setf (aref score-table x y) 0)))

(setf (aref score-table 1 1) 100
      (aref score-table 1 2) 200
      (aref score-table 1 3) 1000
      (aref score-table 1 4) 1100
      (aref score-table 1 5) 1200

      (aref score-table 2 3) 200

      (aref score-table 3 3) 300

      (aref score-table 4 3) 400

      (aref score-table 5 1) 50
      (aref score-table 5 2) 100
      (aref score-table 5 3) 500
      (aref score-table 5 4) 550
      (aref score-table 5 5) 600

      (aref score-table 6 3) 600)

(defun score (&rest dice)
  (when (equal dice nil) (return-from score 0))
  (setf num-count (make-array '(7) :initial-contents '(0 0 0 0 0 0 0)))
  (dolist (n dice)
    (incf (aref num-count n)))
  (loop for i from 1 to 6
        sum (aref score-table i (aref num-count i))))
